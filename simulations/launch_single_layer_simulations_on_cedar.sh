...
cat <<END_OF_SCRIPT > model_script_single_layer.pbs
#!/bin/bash
# ---------------------------------------------------------------------
...
sed -i 's,NAME1,'"${network_name}"',g'                                        model_script_single_layer.pbs
sed -i 's,OBSERVATION_DEPTH_L,'"${observation_depth_L}"',g'                   model_script_single_layer.pbs
sed -i 's,APP_COVERAGE,'"${app_coverage}"',g'                                 model_script_single_layer.pbs
sed -i 's,FRACTION_OF_PRIVATE_PROFILES,'"${fraction_of_private_profiles}"',g' model_script_single_layer.pbs
sed -i 's,NB_SIMULATIONS,'"${nb_simulations}"',g'                             model_script_single_layer.pbs
sed -i 's,OUTPUT_FILENAME,'"${output_filename}"',g'                           model_script_single_layer.pbs
sed -i 's,START,'"${start}"',g'                                               model_script_single_layer.pbs
sed -i 's,STEP,'"${step}"',g'                                                 model_script_single_layer.pbs
sed -i 's,STOP,'"${stop}"',g'                                                 model_script_single_layer.pbs
sed -i 's,TIME,'"${time}"',g'                                                 model_script_single_layer.pbs

#  Submits the task.
sbatch model_script_single_layer.pbs

# Deletes the script.
rm model_script_single_layer.pbs
